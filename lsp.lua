...
local function setup_autocommands(client, buf)
	if not client.supports_method(M.textDocument_hover) then
		augroup(("LspHoverDiagnostics%d"):format(buf), {
			...
			command = function()
				if not rvim.lsp.hover_diagnostics.enable then
					return
				end
			...
			command = function(args)
				...
				then
					local clients = vim.tbl_filter(function(c)
						return c.supports_method(M.textDocument_formatting)
...
function rvim.lsp.notify(msg, type)
	vim.schedule(function()
		vim.notify(msg, type, { title = "Diagnostic Toggles" })
	end)
end
...
local function toggle_hover_diagnostics()
	...
	rvim.lsp.notify(string.format("hover diagnostics %s", bool2str(rvim.lsp.hover_diagnostics.enable)))
end
