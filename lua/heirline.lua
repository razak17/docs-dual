...
local function setup_colors()
	...
	if ar_config.colorscheme == "onedark" then
...
ar.augroup("Heirline", {
	event = "ColorScheme",
	command = function()
		if not ar.is_available("heirline.nvim") then
			return
		end
...
return {
	{
		...
		config = function(_, opts)
			...
			opts.statusline = {
				...
				-- Git
				{
					...
					},
					{
						...
						{
							condition = function()
								return GitStatus.status == "pending"
							end,
				...
				utils.insert(
					file_block,
					utils.insert(statusline.pretty_path, statusline.file_flags, statusline.file_size)
				),
				-- Python env
				{
					condition = function()
				...
				-- LSP Progress
				{
					...
					condition = function()
						return not minimal and ar.lsp.enable
					end,
					provider = function()
				...
				{
					condition = function()
						return ar.is_available("noice.nvim") and require("noice").api.status.command.has()
					end,
					provider = function()
				...
				{
					condition = function()
						return fn.expand("%") == "package.json"
					end,
					provider = statusline.package_info,
					hl = { fg = "comment" },
					on_click = {
						callback = function()
							require("package_info").toggle()
						end,
						name = "update_plugins",
					},
				},
				...
				-- cloc
				{
					...
					condition = function()
						return ar.is_available("cloc.nvim")
					end,
					provider = function(_)
				...
				-- LSP Clients (null-ls)
				{
					...
					provider = function()
						return " " .. statusline.lsp_client_names()
					end,
					hl = { bold = true },
					on_click = {
						callback = function()
				...
				-- LSP Clients (conform,nvim-lint)
				{
					...
					init = function(self)
						...
						self.clients = vim.tbl_filter(function(client)
							return client.name ~= "copilot"
						end, self.clients)
						if falsy(self.clients) then
							self.active = false
						end
					...
					end,
					{
						...
						init = function(self)
							if self.active then
								...
								self.client_names = vim.iter(ipairs(lsp_servers))
									:map(function(_, c)
										return c.name
									end)
									:totable()
						...
						provider = function(self)
							...
							if #self.client_names > 1 then
								...
								return " " .. fmt("%s +%d", self.client_names[1], #self.client_names - 1) .. separator
							end
							return " " .. self.servers
						end,
						hl = { bold = true },
					...
					},
					{
						...
						condition = function(self)
							return not ar.falsy(self.linters)
						end,
						provider = function(self)
							return " " .. self.linters
						end,
					...
					},
					{
						update = { "LspAttach", "LspDetach", "WinEnter" },
						condition = function(self)
							return not ar.falsy(self.formatters)
						end,
						provider = function(self)
				...
				-- Formatting
				{
					...
					provider = function()
						return "  " .. codicons.misc.shaded_lock
					end,
				...
				{
					condition = function()
						return vim.wo.spell and false
					end,
					provider = function()
						return " " .. icons.misc.spell_check
					end,
					hl = { fg = "blue" },
				},
				-- Treesitter
				{
					condition = function()
						return ar.treesitter.enable and false
					end,
					provider = function()
						return "  " .. statusline.ts_active()
					end,
					hl = { fg = "forest_green" },
				},
				-- Session
			...
			opts.statuscolumn = {
				condition = function()
					if not ar_config.ui.statuscolumn.enable or ar_config.ui.statuscolumn.variant ~= "plugin" then
						return false
					end
					local win = api.nvim_get_current_win()
					local buf = api.nvim_win_get_buf(win)
				...
				end,
				{
					...
					{
						provider = function(self)
							return self.no and "" or self.left
						end,
						on_click = {
							name = "sign_click",
					...
					-- Git signs
					{
						{
							condition = function()
								return not is_git_repo() or v.virtnum ~= 0
							end,
							provider = space(),
						...
						{
							condition = function()
								return is_git_repo() and v.virtnum == 0
							end,
							init = function(self)
					...
					-- Virtual lines
					{
						...
						provider = function(self)
							if self.is_shade then
								return ""
							end
							if not self.is_wrap then
								return sep.left_thin_block
							end
							return ""
						end,
					...
					-- Folds
					{
						...
						init = function(self)
							self.fold = statuscol.get_fold(v.lnum)
						end,
						{
							provider = function(self)
